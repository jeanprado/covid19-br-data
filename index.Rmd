...
covid <- read_csv2(url, locale=locale(encoding="Latin1")) %>% janitor::clean_names() %>%
  rename(uf=2, data=3, casos=casos_acumulados, obitos=obitos_acumulados) %>%
...
summarize_prop <- function(.data) .data %>% summarize_casos() %>% 
  mutate(casos_prop=casos/pop*100000, casos_novos_prop=casos_novos/pop*100000,
         obitos_novos_prop=obitos_novos/pop*100000, obitos_prop=obitos/pop*100000)
...
covid %>% group_by(data) %>% summarize_casos() %>%  
  ...
  { ggplot(., aes(data, n, color=type)) + time_series() + labs(x="", y="") +
      geom_text(data=filter(., type %in% c('casos_novos', 'casos') | type == "obitos" & n>600, n>300),
