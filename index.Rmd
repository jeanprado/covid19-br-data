...
w_covid <- read_csv('https://covid.ourworldindata.org/data/ecdc/full_data.csv') %>% 
  ...
  ungroup() %>% mutate_at(vars(10:13), ~na_if(., -1)) %>% 
...
br_cases <- w_covid %>% filter(date==last(date), location=="Brasil") %>% arrange(desc(total_cases))

top_countries <- w_covid %>% filter(cases_100==br_cases$cases_100, !is.na(continent)) %>% 
  arrange(desc(total_cases)) %>% head(5)

text_countries <- paste0('**', top_countries$location, '** (', trimws(th(top_countries$total_cases)), ')')
```

no **`r br_cases$cases_100`º dia desde o 100º caso**, o Brasil tem **`r th(br_cases$total_cases)` casos confirmados de Covid-19**. para comparação, os **5 países com mais casos no mundo** tinham, neste período, esse número de casos: `r  text_countries`.
...
w_covid %>% filter(location %in% c('Estados Unidos', 'Espanha', 'Itália', 'Alemanha', 'França', 'China',
  ...
  ggplot(aes(cases_100, total_cases, group=location, color=br_color)) +
  geom_line(aes(size=is_br, alpha=is_br)) + scale_y_log10(limits=c(100, 10e5), labels=function(x) th(x)) +
  remove_lines + theme(legend.position='top', title=element_text(size=12)) +
  scale_x_continuous(n.breaks=10) + labs(x="", y="", color="", title="brasil e mundo: dias desde o 100º caso") +
  geom_text_repel(data=. %>% filter(date==last(date), location!="brasil"), aes(label=location),
                  vjust=.4, hjust=-.8, segment.size=NA, seed=0) + guides(size=F, alpha=F) +
...
```{r table_deaths, include=FALSE}
# no **`r br_cases$deaths_5`º dia** desde o 5º óbito, o Brasil tem **`r th(br_cases$total_deaths)` óbitos decorrente da Covid-19**. para comparação, os **5 países com mais mortes no mundo** tinham, neste período, esse número de óbitos:

# top_countries %>% arrange(desc(total_deaths)) %>% head(5) %>% select(location, date, total_deaths) %>% mutate(date=format(date, "%d/%m"), total_deaths=th(total_deaths)) %>% DT::datatable()
```
